...
class Solution:
    def findMaximumElegance(self, items: List[List[int]], k: int) -> int:
        def grace(profit: int, categories: set[int]) -> int:
            return profit + len(categories) ** 2

        stack = set()
        dictionary = {}
        category = set()
        profits = 0
        # 用一个最小堆来维护前k个元素，按照profit排序
        heap = []

        # 初始处理前k个元素
        for ind, (i, j) in enumerate(items[:k]):
            stack.add(ind)
            dictionary[ind] = (i, j)
            category.add(j)
            profits += i
            heapq.heappush(heap, (i, ind))

        init_grace = grace(profits, category)
        for ind, (i, j) in enumerate(items[k:]):
            # 如果新的元素不会导致category+1，则只pop出最小的profit
            if j in category:
                smallest_profit, smallest_ind = heapq.heappop(heap)
